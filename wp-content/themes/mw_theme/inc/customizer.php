 ...
 */
function mw_theme_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'theme_bg_color', 
				array(
					'label'		=> __( 'Header and footer background color', 'mw_theme'),
					'section'	=> 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'interactive_color', array(
				'label'		=> __( 'Interactive color (links etc)', 'mw_theme' ),
				'section'	=> 'colors',
	...
	$wp_customize->add_section( 'theme_options', 
		array(
			'title'			=> __( 'Theme Options', 'mw_theme' ),
			'priority'		=> 95,
			'capability'	=> 'edit_theme_options',
			'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'mw_theme' )
		)
	);
	...
	$wp_customize->add_setting(	'length_setting',
		array(
			'default'			=> 'excerpt',
			'type'				=> 'theme_mod',
			'sanitize_callback' => 'mw_theme_sanitize_length', // Sanitization function appears further down
			'transport'			=> 'postMessage'
		)
	);

	// Add the controls
	$wp_customize->add_control(	'mw_theme_length_control',
		array(
			'type'		=> 'radio',
			'label'		=> __( 'Index/archive displays', 'mw_theme' ),
			'section'	=> 'theme_options',
			'choices'	=> array(
				'excerpt'		=> __( 'Excerpt (default)', 'mw_theme' ),
				'full-content'	=> __( 'Full content', 'mw_theme' )
			),
...
add_action( 'customize_register', 'mw_theme_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function mw_theme_customize_preview_js() {
	wp_enqueue_script( 'mw_theme_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'mw_theme_customize_preview_js' );
...
function mw_theme_sanitize_length( $value ) {
...
if ( ! function_exists( 'mw_theme_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @see mw_theme_custom_header_setup().
 */
function mw_theme_header_style() {
